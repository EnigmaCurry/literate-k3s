...
repository to the path structure suggested in the introduction guide, this
setting should already be correct. If you cloned to a different directory,
should update this variable accordingly, /however/, don't change this directory
to anything other than the one that contains this file, because org-mode is not
configured for tangling to any other directory.
...
Create all of the directories necessary for tangling this config. (These are
also re-iterated in the setup instructions for each individual config below, but
this gets you going quicker, because tangling will fail until you create all of
these):
