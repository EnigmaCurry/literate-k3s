...
Open =${HOME}/git/literate-k3s/config.org=.

 * =config.org= contains key configuration variables for use across your entire
   cluster.
 * You should review all of the code blocks in =config.org=, and edit those
   variables appropriately for your environment, especially =CLUSTER=.

Open =${HOME}/git/literate-k3s/traefik.org=

 * Edit =TRAEFIK_ACME_EMAIL=, =TRAEFIK_WHOAMI_DOMAIN=, and any other variables
   according to your needs.
 * Follow the other steps as outlined: make the directory, deploy Traefik, and
