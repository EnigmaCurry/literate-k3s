...
export HTML whenever the file is saved, see =Local Variables=. You can toggle
this behaviour on and off, by executing =M-x toggle-org-html-export-on-save=.
...
configuration when it loads that other files rely upon. See =Local Variables=.)
